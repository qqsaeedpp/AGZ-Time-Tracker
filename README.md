...
## شخصی‌سازی دکمه‌ها (متن / ایموجی / رنگ)

از پنل مدیریت → **🔘 شخصی‌سازی دکمه‌ها**: ابتدا دکمهٔ موردنظر و سپس بخش قابل‌تغییر
(متن، ایموجی موضوعی، یا رنگ) را انتخاب کنید. هر دکمه به‌صورت `{رنگ} {ایموجی} {متن}`
ساخته می‌شود و کیبورد به‌صورت داینامیک از دیتابیس خوانده می‌شود.

- رنگ پیش‌فرض دکمه‌ها طبق اسپک: گزارش‌ها 🔵، تاریخ امروز 🟢، پشتیبانی 🔴.
- ایموجی موضوعی اختیاری است؛ با ارسال «حذف» برداشته می‌شود.
- **دکمه‌های inline از ایموجی پریمیوم پشتیبانی نمی‌کنند** — فقط ایموجی معمولی.

دیتای ذخیره‌شده در جدول `button_settings`: `button_key`، `button_text`،
`button_emoji`، `button_color` (هر ستون NULL به مقدار پیش‌فرض برمی‌گردد).
